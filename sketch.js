    ...
    if (relMouseX >= 0 && relMouseX <= menuWidth) {
        ...
        for (let i = 0; i < menuItems.length; i++) {
            ...
            if (mouseY >= y && mouseY <= y + 48) {
                ...
                if (i === 0) {
                    // 第一單元作品：以 iframe 顯示指定網址
                    showIframe('https://acgn1221-ai.github.io/20251020/');
                } else if (i === 1) {
                    // 第一單元講義：以 iframe 顯示 HackMD 頁面
                    showIframe('https://hackmd.io/@oQlKWzw8Sb6Vp8HMwRikbg/Byu6UQCjxe');
                } else {
                    // 其他選項：隱藏 iframe（或改為其他行為）
                    hideIframe();
                    // 若要導頁可使用： location.href = '...';
                }
            }
        }
    } else {
